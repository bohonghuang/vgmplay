            ...
            (block player-widget
              ...
              (when (plusp
                     (raygui:button (raylib:make-rectangle :x (coerce pos-x 'single-float)
                                    ...
                                    (raygui:icon-text #.(foreign-enum-value 'raygui:icon-name :file-open) "Open")))
              ...
              (when (plusp
                     (raygui:button (raylib:make-rectangle :x (coerce pos-x 'single-float)
                                    ...
                                    (if (or (not (agbplayer-playing-p player)) (agbplayer-paused-p player))
                                        (raygui:icon-text #.(foreign-enum-value 'raygui:icon-name :player-play) "Play")
                                        (raygui:icon-text #.(foreign-enum-value 'raygui:icon-name :player-pause) "Pause"))))
              ...
              (when (plusp
                     (raygui:button (raylib:make-rectangle :x (coerce pos-x 'single-float)
                                                           ...
                                                           :width 100.0 :height 25.0)
                                    (raygui:icon-text
                                     #.(foreign-enum-value 'raygui:icon-name :file-export)
                                     (if (bt2:thread-alive-p export-thread)
            ...
            (when (plusp
                   (raygui:button
                    (raylib:make-rectangle :x (coerce pos-x 'single-float)
                                           :y (coerce pos-y 'single-float)
                                           :width 100.0 :height 25.0)
                    (raygui:icon-text #.(foreign-enum-value 'raygui:icon-name :exit) "Exit")))
              (throw 'exit nil))
            (incf pos-y 30)
